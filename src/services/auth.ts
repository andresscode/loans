...
export const authService = {
  ...
  async setup(username: string, password: string, displayName: string): Promise<ActionResult<User>> {
    return window.api.auth.setup({ username, password, displayName });
  },

  async login(username: string, password: string): Promise<ActionResult<{ user: User; token: string }>> {
